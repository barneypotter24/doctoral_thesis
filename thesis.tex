...
\cosupervisor{Prof.~Dr.~M.S.~Gill}{}
\president{Prof.~Dr.~K.~Van~Laethem}
\jury{Prof.~Dr.~P.~Lemey\\
      Prof.~Dr.~J.~Matthijnssens\\
}
\externaljurymember{Dr.~Bram~Vrancken}{\textit{Universit\'{e} Libre de Bruxelles}}
\externaljurymember{Dr.~Verity~Hill}{\textit{Yale University}}
...
\usepackage[square,numbers,sort&compress]{natbib} % For more control over bibliography and in-line citation formatting

\usepackage{caption} % For figure caption font size control
...
\newcommand\xu{\rotatebox[origin=c]{180}{$\pi$}}
...
% This bit helps draft compilation time
% Remove from final draft or comment any time I want to see the figures included
% \newif\ifdropfigures  % At the time of definition it is false!
% \dropfigurestrue % yes, we drop figures
% % \dropfiguresfalse % LEAVE THIS COMMENTED EVEN IF THE REST IS UNCOMMENTED
% \ifdropfigures
...
\newglossaryentry{mcc}{name={MCC},description={Maximum clade credibility (tree). Given a set of posterior trees, the individual tree from the set whose internal nodes---defined by their descendant tips---occur in the highest proportion of the distribution of trees}}
...
\newglossaryentry{hky}{name={HKY},description={Hasegawa-Kishino-Yano model of nucleotide evolution. Markov model of nucleotide evolution in which the rate of transitions ($A \iff G$ or $C \iff T$) share one rate and transversions ($A \iff C/T$ or $G \iff C/T$) share another. The transition/transversion ratio can be described by a single parameter, kappa ($\kappa$)}}
\newglossaryentry{hpd}{name={HPD},description={Highest posterior density (interval). The smallest interval that encompases a given percent of posterior density that is used to express the range of estimated uncertainty}}
\newglossaryentry{orf}{name={ORF},description={Open reading frame. Area of a genome between start and stop codons in which may be transcribed and translated}}
\newglossaryentry{hiv}{name={HIV},description={Human immunodeficiency virus. Ont of two species of retroviruses that are the causative agent of acquired immunodeficiency syndrome (AIDS). Responsible for the HIV/AIDS pandemic}}
\newglossaryentry{hcv}{name={HCV},description={Hepatitis C virus}}
...
\newglossaryentry{msa}{name={MSA},description={Multiple sequence alignment. Set of nucleotide sequences in FASTA format that have had their constituent nucleotides shifted according to an alignment algorithm (as implemented in software such as MAFFT or nextalign) so that the sequences are in frame with one another and can be compared at each site. Often constructed in relation to a given reference genome. The alignment is the primary input into phylogenetic reconstruction software used in this thesis}}
...
\newglossaryentry{sarscov2}{name={SARS-CoV-2},description={Severe acute respiratory syndrome coronavirus 2. The causative agent of COVID-19 and the pathogen responsible for the global COVID-19 pandemic that began in spring 2020. One of the two viruses being analyzed as part of this thesis, along with HBV}}
\newglossaryentry{voc}{name={VOC},description={Variant of concern. SARS-CoV-2 lineage which demonstrates both fitness-improving mutations in the spike protein, and increased spread in the global population compared to previously circulating variants. Frequently referred to by their WHO Greek-letter name (e.g. Alpha, Delta, Gamma, etc.)}}
\newglossaryentry{voi}{name={VOI},description={Variant of interest. SARS-CoV-2 lineage which harbors known or speculative fitness-improving mutations in the spike protein. This designation is typically assigned prior to WHO designation as a variant of concern (VOC). Usually referred to by their Pango lineage designation (e.g. B.1.1.7, BA.1, etc.)}}
\newglossaryentry{ecdc}{name={ECDC},description={European Centre for Disease Prevention and Control}}
\newglossaryentry{pcr}{name={PCR},description={Polymerase chain reaction. A method used for the amplification of genetic material so that it may be used for both diagnostics (i.e PCR testing for COVID-19), or for whole genome sequencing}}
...
\begin{appendices}
% \includeappendix{appendix_ch1} % double commented because it should be permanently excluded
\includeappendix{appendix_ch2}
\includeappendix{appendix_ch3}
\includeappendix{appendix_papers}
\includeappendix{appendix_ai}
\end{appendices}
